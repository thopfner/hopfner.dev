...
export function PageEditor({ pageId }: { pageId: string }) {
  ...
  async function updatePageSettings(patch: Partial<Pick<CmsPageRow, "bg_image_url" | "formatting_override">>) {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to update page settings.")
    } finally {
      setPageSaving(false)
    }
  }
  ...
  async function onApplyBackdropToAllPages() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to apply page backdrop settings to all pages.")
    } finally {
    ...
    <Stack gap="md">
      ...
      <Paper withBorder p="md" radius="md">
        <Stack gap="sm">
          ...
          <Stack gap={4}>
            <Text size="sm" fw={600}>Nav overlay opacity</Text>
            <Text size="xs" c="dimmed">{Math.round(pageNavOpacityDraft * 100)}%</Text>
            <Slider
              label={(v) => `Nav overlay opacity ${Math.round(v * 100)}%`}
              min={0}
              max={0.6}
              step={0.02}
          ...
          <Stack gap={4}>
            <Text size="sm" fw={600}>Background image opacity</Text>
            <Text size="xs" c="dimmed">{Math.round(pageBgImageOpacityDraft * 100)}%</Text>
            <Slider
              label={(v) => `Background image opacity ${Math.round(v * 100)}%`}
              min={0}
              max={1}
              step={0.01}
          ...
          <Stack gap={4}>
            <Text size="sm" fw={600}>Panel/card surface opacity</Text>
            <Text size="xs" c="dimmed">{Math.round(pagePanelOpacityDraft * 100)}%</Text>
            <Slider
              label={(v) => `Panel/card surface opacity ${Math.round(v * 100)}%`}
              min={0}
              max={1}
              step={0.01}
      ...
      >
        <Stack gap="sm">
          ...
          <Group justify="end">
            ...
            <Button color="blue" loading={applyBackdropLoading} onClick={() => void onApplyBackdropToAllPages()}>
