...
Service role key is required for privileged server-side API operations (media admin, migrations, and blueprint apply/rollback endpoints).
...
## 7) Blueprint Content Apply / Rollback API

Admin-only backend routes are available for safe content ingestion from the blueprint:

- `GET /admin/api/content/blueprint` → plan/preview metadata
- `POST /admin/api/content/blueprint` → applies content (creates snapshot first)
  - optional JSON body: `{ "dryRun": true }`
- `POST /admin/api/content/blueprint/rollback` → rollback using snapshot id
  - JSON body: `{ "snapshotId": "<uuid>" }`

Safety behavior:
- Every apply stores a full snapshot in `public.cms_content_snapshots`
- Rollback restores pages/sections/versions from the snapshot payload
- Endpoints require authenticated admin user + service-role-backed server access

## 8) Deploy Behind Nginx (Design Assumption)
